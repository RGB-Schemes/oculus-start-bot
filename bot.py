        ...
        if startParser.exists and startParser.isOculusStartMember:
            if str(startParser.discordUsername) == str(ctx.author):
                ...
                if startParser.discordUsername == None:
                    embed.add_field(name=":x:", value="Couldn't find a comment from {0} with a Discord handle! Please comment on [your profile]({1}) with the **exact** Discord handle (it is case sensative!) See the below image for how this should look:".format(forumUsername, addr))
            ...
            if startParser.exists == False:
                embed.add_field(name=":x:", value="{0} is **NOT** a valid Oculus username! Please enter a valid Oculus username as the input parameter! See this image for where to find your Oculus username:".format(forumUsername, addr))
